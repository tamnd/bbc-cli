...
describes what users get. Pick a format with `-o`, or let bbc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bbc <command> -o table   # aligned columns for reading
bbc <command> -o jsonl   # one JSON object per line, for piping
bbc <command> -o json    # a single JSON array
bbc <command> -o csv     # spreadsheet friendly
bbc <command> -o tsv     # tab-separated
bbc <command> -o url     # just the URL column
bbc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bbc <command> --fields id,title,url
```
...
```bash
bbc <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
bbc <command>            # a table, because this is a terminal
bbc <command> | wc -l    # JSONL, because this is a pipe
```
