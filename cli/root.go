// Package cli assembles the bbc command tree from the bbc
...
// NewApp assembles the kit application from the bbc domain. The
...
//
// To add a command, declare it in bbc/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := bbc.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(bbc.Domain{}).Register(app)
