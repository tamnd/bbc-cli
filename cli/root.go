// Package cli builds the bbc command tree on top of the bbc library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: bbc.DefaultConfig()}

	root := &cobra.Command{
		Use:   "bbc",
		Short: "Read BBC News stories by section.",
		Long: `bbc reads BBC News through the official public RSS feeds at feeds.bbci.co.uk.
No API key or authentication is required. It returns records as table, JSON,
JSONL, CSV, TSV, or URLs.

bbc is an independent tool and is not affiliated with the BBC.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	root.AddCommand(
		app.sectionCmd("top", "Top/home BBC News stories", "/news/rss.xml", 20),
		app.sectionCmd("world", "BBC World News", "/news/world/rss.xml", 20),
		app.sectionCmd("uk", "BBC UK News", "/news/uk/rss.xml", 20),
		app.sectionCmd("tech", "BBC Technology news", "/news/technology/rss.xml", 20),
		app.sectionCmd("science", "BBC Science and Environment", "/news/science_and_environment/rss.xml", 20),
		app.sectionCmd("health", "BBC Health news", "/news/health/rss.xml", 20),
		app.sectionCmd("business", "BBC Business news", "/news/business/rss.xml", 20),
		app.sectionCmd("entertainment", "BBC Entertainment and Arts", "/news/entertainment_and_arts/rss.xml", 20),
		app.sectionCmd("sport", "BBC Sport", "/sport/rss.xml", 20),
		app.sectionCmd("politics", "BBC Politics news", "/news/politics/rss.xml", 20),
		app.feedCmd(),
		app.sectionsCmd(),
		newVersionCmd(),
	)
